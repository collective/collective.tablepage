...
class TablePage(Item):
    ...
    def getTextBefore(self):
        return self.textBefore and self.textBefore.output or ''

    def getTextAfter(self):
        return self.textAfter and self.textAfter.output or ''

    def getDownloadEnabled(self):
        return self.downloadEnabled

    def getAttachmentStorage(self):
        if not self.attachmentStorage:
            return aq_inner(self)
        if self.attachmentStorage and self.attachmentStorage.to_object:
            return self.attachmentStorage.to_object
        return
